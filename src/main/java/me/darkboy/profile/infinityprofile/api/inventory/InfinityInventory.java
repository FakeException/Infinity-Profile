package me.darkboy.profile.infinityprofile.api.inventory;
...
import me.darkboy.profile.infinityprofile.api.inventory.opener.InventoryOpener;
...
public class InfinityInventory {
    ...
    private InfinityInventory parent;

    private List<InventoryListener<? extends Event>> listeners;
    private InventoryManager manager;

    private InfinityInventory(InventoryManager manager) {
        this.manager = manager;
    }

    public Inventory open(Player player) { return open(player, 0); }
    public Inventory open(Player player, int page) {
        Optional<InfinityInventory> oldInv = this.manager.getInventory(player);
    ...
    public Optional<InfinityInventory> getParent() { return Optional.ofNullable(parent); }
    ...
    public static final class Builder {
        ...
        public Builder parent(InfinityInventory parent) {
            this.parent = parent;
            return this;
        }
        ...
        public InfinityInventory build() {
            if(this.provider == null)
                throw new IllegalStateException("The provider of the InfinityInventory.Builder must be set.");

            InventoryManager manager = this.manager != null ? this.manager : InfinityPlugin.manager();

            if(manager == null)
                throw new IllegalStateException("The manager of the InfinityInventory.Builder must be set, "
                        + "or the SmartInvs should be loaded as a plugin.");

            InfinityInventory inv = new InfinityInventory(manager);
            inv.id = this.id;
